...
static int led_cdev_open(struct inode *inode, struct file *filp)
{
	...
	cur_led= container_of(inode->i_cdev, struct led_data, led_cdev);
...
static int led_pdrv_probe(struct platform_device *pdev)
{
	...
	register_chrdev_region(cur_dev, 1, "beep_pdev");
	...
	device_create(my_led_class, NULL, cur_dev, NULL, DEV_NAME);
